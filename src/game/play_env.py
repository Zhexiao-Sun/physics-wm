...
from controls.action_processing import PIWMAction, decode_action, encode_action, print_action
from controls.keymap import PIWM_KEYMAP
...
class PlayEnv:
    ...
    @torch.no_grad()
    def step(self, action_input: PIWMAction) -> Tuple[Tensor, Tensor, Tensor, Tensor, Dict[str, Any]]:
        if self.is_human_player:
            # Use 51-dim one-hot; only first 5 indices map to discrete highway actions
            keys = [] if action_input is None else action_input.key_names
            # Map to 5-key indices within 51-dim vector
            if ("a" in keys) or ("left" in keys):
                ...
                action_idx = 1  # default medium

            action = torch.zeros(51, dtype=torch.long, device=self.agent.device)
            action[action_idx] = 1
        else:
            action = self.env.next_act[self.t - 1] if self.t > 0 else self.env.act_buffer[0, -1].clone()
            action_input = decode_action(action.cpu())
        ...
        keys, mouse, clicks = print_action(action_input)
        ...
        key_names = [] if action_input is None else action_input.key_names
        keys_state = {
            "w": ("w" in key_names),  
            "a": ("a" in key_names),  
            "s": ("s" in key_names),  
        ...
        info = {"header": header, "keys_state": keys_state}
